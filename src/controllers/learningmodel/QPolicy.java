...
public class QPolicy {
    ...
    public int getActionNoExplore(double[] feature, StateObservation obs) throws Exception {
        double[] Q = getQArray(feature, obs);
    ...
    public int getAction(double[] feature, StateObservation obs) throws Exception {
        double[] Q = getQArray(feature, obs);
    ...
    public double getMaxQ(double[] feature, StateObservation obs) throws Exception {
        double[] Q = getQArray(feature, obs);
    ...
    public double[] getQArray(double[] feature, StateObservation stateObs) throws Exception {
        ...
        for (int action = 0; action < m_numActions; action++) {
            ...
            Types.ACTIONS actioN = Agent.action_mapping.get(action);
...
//            if (eff == 0 || alive == 0) {
//                Q[action] = alive == 0 ? -10000000 : -10000 + m_rnd.nextInt(10000);
//                continue;
//            }
            feature[feature.length - 4] = eff;
            feature[feature.length - 3] = alive;
    ...
    public void fitQ(Instances data) throws Exception {
        if (m_c == null) {
//            m_c = new weka.classifiers.lazy.IBk(3);
